...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url }) => {
      ...
      if (debug) console.log('[auth.debug] sendResetPassword called', { email: user?.email, url })
      ...
      try {
        ...
        if (debug) console.log('[auth.debug] sendResetPassword result', result)
        
        if (!result.success) {
          console.error('[auth.error] reset password email failed to send:', result.error);
        }
      } catch (error) {
        console.error('[auth.error] reset password email exception:', error);
      }
    }
  },
  plugins: [],
